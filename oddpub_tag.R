...
oddpub_tag <- function(id, path){
  
  # Run tokenisation
  PDF_text_sentences <- purrr::map(path, readr::read_lines) %>% 
  ...
  # Get names
  names(PDF_text_sentences) <- id
  message(paste0("Sucessfully loaded ", length(names(PDF_text_sentences)), 
                 " text files"))
  
  # Run open data / code tag
  ...
  # Format results
  open_data_results <- t(open_data_results)
  open_data_results <- as.data.frame(open_data_results)
